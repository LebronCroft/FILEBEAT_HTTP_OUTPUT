...
func main() {
	...
	agentID, err := filebeatConfig.GetAgentID(enum.AgentIDAddress)
	if err != nil {
	...
	infraLog.GlobalLog.Info(fmt.Sprintf("Starting to get Agent ID %s", agentID))
	...
	for {
		...
		err = filebeatConfig.UpdateFilebeatConfig(pluginsTask.Data, wd+enum.ModuleFileBeatConfigFile)
		if err != nil {
			return
		}
		infraLog.GlobalLog.Info("Filebeat config generated successfully.")
